...
<body>
    ...
    <div class="container mt-5">
        <div class="categories">
            ...
            <!--CARD SEGUN CATEGORIA-->
                <!--Solo mostrar si se elige una categoria, cuando muestra todo el vocabulario no mostrar esta tarjeta-->

                <?php
                    if($ejecutar = mysqli_query($varConexion, "SELECT * FROM category WHERE idCategory= '".$consultaV."' ") ){
                        while ($row = $ejecutar -> fetch_array (MYSQLI_NUM)){
                            $e = array();
                ?>
                    <div class="categoria shadow-sm py-3 px-4 bg-body-tertiary ">
                        ...
                        <div class="textCategoria">
                            ...
                            <?php
                                if($ejecutar = mysqli_query($varConexion, "SELECT count(*) FROM vocabulary WHERE idCat = '".$consultaV."' " ) ){
                                    while ($row = $ejecutar -> fetch_array (MYSQLI_NUM)){
                                        $e = array();
                            ?>
    ...
    <!--Cards-->
        <div class="container mt-5">
            <div class="row">
                <?php
                // Verifica si el formulario ha enviado algo por POST
                ...
                if (!empty($consultaPalabra)) {
                    // Si se ha ingresado una palabra, busca en el campo 'word'
                    $query = "SELECT * FROM vocabulary WHERE word LIKE '%" . $consultaPalabra . "%'";
                } else {
                    // Si no hay palabra ingresada, busca por categoría
                    $query = "SELECT * FROM vocabulary WHERE idCat= '".$consultaV."'";
                }
